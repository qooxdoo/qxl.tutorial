...
qx.Class.define("qxl.tutorial.Application", {
  ...
  members: {
    ...
    main() {
      ...
      // Create header
      this.__header = new qxl.tutorial.view.Header();
      this.__header.addListener(
        "selectTutorial",
      ...
      this.__description = new qxl.tutorial.view.Description();
    ...
    openSelectionWindow() {
      if (!this.__selectionWindow) {
        this.__selectionWindow = new qxl.tutorial.view.SelectionWindow(
          this.__desktopTutorials,
          this.__mobileTutorials
        );

        this.__selectionWindow.addListener(
          "changeTutorial",
          this.__onChangeTutorial,
          this
        );
      }
    ...
    updateEditor(e) {
      ...
      this.confirm(
        "This will replace the current code in the editor.",
        function (ok) {
          if (ok.getData()) {
    ...
    confirm(text, callback, ctx) {
      if (!this.__confirmWindow) {
        this.__confirmWindow = new qxl.tutorial.view.Confirm();
      }
    ...
    loadTutorial(name, type) {
      ...
      req.addListener(
        ...
        function (e) {
          ...
          this.__playArea.updateCaption(
            name.replace(/_/g, " ") + " (" + type + ")"
          );
          this.__playArea.setMode(type !== "desktop" ? "mobile" : "ria");
          this.__actionArea.setOrientation(
            type == "desktop" ? "vertical" : "horizontal"
          );
        },
        this
      );
      ...
      req.addListener(
        ...
        function (evt) {
          ...
          if (window.location.protocol == "file:") {
            ...
            alert(
              "Failed to load the tutorials from the file system.\n\n" +
