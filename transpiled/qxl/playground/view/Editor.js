  ...
  qx.Class.define("qxl.playground.view.Editor", {
    ...
    members: {
      ...
      init: function init() {
        ...
        caption.add(this.__errorLabel); // plain text area
       ...
       */
      __onEditorAppear: function __onEditorAppear() {
        // timout needed for chrome to not get the ACE layout wrong and show the
        // text on top of the gutter
        qx.event.Timer.once(function () {
          var container = this.__editor.getContentElement().getDomElement(); // create the editor


          var editor = this.__ace = ace.edit(container); // set javascript mode
      ...
      setCode: function setCode(code) {
        if (this.__ace) {
          this.__ace.getSession().setValue(code); // move cursor to start to prevent scrolling to the bottom
      ...
      useHighlight: function useHighlight(value) {
        ...
        if (value) {
          ...
          this.__textarea.setVisibility("excluded"); // copy the value, if the editor already availabe
          ...
          this.__textarea.setVisibility("visible"); // copy the value, if the editor already availabe
...
//# sourceMappingURL=Editor.js.map?dt=1588104177053